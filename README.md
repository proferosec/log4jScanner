...
For every response it receives, it will log the responding host IP to get a list of the vulnerable servers.
...
![example]zhttps://github.com/proferosec/log4jScanner/blob/main/movie.gif)
...
* If a callback happens, this means that a vulnerable server exists, the exploit worked and it initiated a callback. 
...
what we can minimize the search. we can get the list of all successful requests made by the scanner from the log, and the enumerate 
