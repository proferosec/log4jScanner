...
release-windows: release-dir
	...
	echo $(shell shasum -a 256 build/windows/$(PROJECT_NAME)-${VERSION}.exe | cut -f1 -d" ") $(PROJECT_NAME).exe > release/windows.sha256.txt

release-darwin: release-dir
	zip -j release/$(PROJECT_NAME)-darwin.zip build/darwin/$(PROJECT_NAME)-${VERSION}
	echo $(shell shasum -a 256 build/darwin/$(PROJECT_NAME)-${VERSION} | cut -f1 -d" ") $(PROJECT_NAME) > release/darwin.sha256.txt

release-linux: release-dir
	zip -j release/$(PROJECT_NAME)-linux.zip build/linux/$(PROJECT_NAME)-${VERSION}
	echo $(shell shasum -a 256 build/linux/$(PROJECT_NAME)-${VERSION} | cut -f1 -d" ") $(PROJECT_NAME) > release/linux.sha256.txt
